...
contract ViperRouter is IViperRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            IViperPair(ViperLibrary.pairFor(factory, input, output)).swap(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = ViperLibrary.sortTokens(input, output);
            IViperPair pair = IViperPair(ViperLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = ViperLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
